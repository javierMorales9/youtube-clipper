...
def ffmpeg(path: str):
    ...
    try:
        subprocess.run(f"""ffmpeg -i {path}/original.mp4 \
         -codec: copy \
         -hls_time 10 \
         -hls_list_size 0 \
         -f hls \
        ...
        ad = subprocess.run(f"""ffprobe \
          -v error \
          -show_entries format=duration \
          -of default=noprint_wrappers=1:nokey=1 \
         {path}/original.mp4""", text=True);
...
def getResolution(path: str):
    process = subprocess.run(f"""ffprobe -v error -select_streams v:0 -show_entries stream=width,height -of csv=s=x:p=0 {path}/original.mp4""", text=True);
    return process.stdout;

def processSource(sourceId: str):
    print("Source uploaded")
    
    basePath = '../public/source/'
    path = basePath + sourceId
    duration = ffmpeg(path);
